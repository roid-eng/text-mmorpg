...
- [x] 게스트 이메일 연동 UI 구현 (is_guest 버그 수정 포함)
- [x] 인벤토리 모달화 (그리드 + 탭 필터 전체/장비/소비)
- [x] 퀘스트 UI 개선 (대상 몬스터명 표시, 진행 바, 일일/스토리 탭)
- [x] PC 스탯 패널 하단 퀘스트 패널 상시 표시
...
- [ ] PvP 결투장
- [ ] 사운드 효과
      (전투 타격음/스킬음/레벨업/퀘스트 완료, UI 클릭음, on/off 토글)
- [ ] 전투 시스템 개선
      (1번 텍스트 비주얼라이징: 데미지 팝업, 치명타 화면 흔들림, 몬스터 예고 메시지)
      (5번 리듬 탭: 몬스터 예고 타이밍 탭 → 1.5배 대미지/회피 성공)
      (클래스별 공격 속도: 전사 2.0초 / 궁수 1.0초 / 마법사 2.5초 / 성직자 1.5초)
      (몬스터별 공격 속도: 몬스터 특성에 맞게 차등 적용, 보스는 예고 메시지 + 강타)
...
## 장기 플랜
- [ ] AI 노드 생성 시스템
      (AI가 지역 노드 구조 + 보스 젠 위치 자동 생성)
      → 조건: 수익 발생 후 API 비용 확보 시점,
               수동 콘텐츠 제작 속도가 유저 소비 속도를 따라가지 못할 때

*Last updated: 2026-04-17 (17차 세션)*
